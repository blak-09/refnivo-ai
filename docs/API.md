...
| `GET /auth/signed-out?reason=…` | `reason ∈ account-suspended, account-pending, account-rejected, account-missing, session-expired` | Clears the session cookie (a Route Handler may; a Server Component may not) and redirects to `/auth/login?error=<reason>`. Page guards redirect here whenever a cookie no longer maps to a usable account — see `lib/auth/session-state.ts`. |
| `POST /api/uploads/[kind]` | multipart `file`; kind ∈ `product-image`, `brand-logo`, `brand-cover` (brand owner), `creator-image` (creator), `avatar` (any user) | One validated pipeline: auth by scope → rate limit (30 / 10 min) → size ≤ 5 MB → declared type → magic-byte sniff → stored under `<folder>/<owner id>/…` via `STORAGE_PROVIDER` (local / supabase). 503 when uploads are unavailable on the deployment. Returns `{ ok, url }`. |
| `deleteAccountAction(prev, formData)` | `confirmation` (= `DELETE`), `currentPassword` (password accounts) | Deletes the signed-in user's OWN account (id from the session): anonymises personal data, keeps ledger/audit, closes a brand owner's brand, refuses while a payout is open or for admins; bumps `sessionVersion` and signs out to `/auth/login?reason=account-deleted`. Rate-limited 5 / 15 min. |
| `GET /api/cron/email-outbox` | `Authorization: Bearer <CRON_SECRET>` | Retries pending/failed outbox e-mails (counts only). 503 without `CRON_SECRET`, 401 on a bad token. |
